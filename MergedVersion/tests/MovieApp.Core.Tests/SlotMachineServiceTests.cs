...
public sealed class SlotMachineServiceTests
{
    ...
    private sealed class InMemoryMovieRepo : IMovieRepository
    {
        ...
        // CRUD stubs required by merged IMovieRepository interface
        public Task<List<Movie>> GetAllAsync(CancellationToken cancellationToken = default) => Task.FromResult(new List<Movie>());
        public Task<Movie?> GetByIdAsync(int id, CancellationToken cancellationToken = default) => Task.FromResult<Movie?>(null);
        public Task<int> InsertAsync(Movie movie, CancellationToken cancellationToken = default) => Task.FromResult(0);
        public Task<bool> UpdateAsync(Movie movie, CancellationToken cancellationToken = default) => Task.FromResult(true);
    ...
    public async Task SpinAsync_HasAvailableSpins_ConsumesSpinAndGrantsRewardForJackpot()
    {
        ...
        SlotMachineService service = new SlotMachineService(stateRepo, movieRepo, eventRepo, discountRepo, new Mock<INotificationRepository>().Object);
    ...
    public async Task RecordLoginAndCheckStreakAsync_ThirdConsecutiveLogin_GrantsSpinAndResetsStreak()
    {
        ...
        InMemoryStateRepo stateRepo = new InMemoryStateRepo();
        SlotMachineService service = new SlotMachineService(stateRepo, new InMemoryMovieRepo(), new InMemoryEventRepo(), new InMemoryDiscountRepo(), new Mock<INotificationRepository>().Object);
    ...
    public async Task SpinAsync_ThrowsInvalidOperationException_WhenNoValidCombinations()
    {
        ...
        mockMovieRepo.Setup(r => r.GetGenresAsync(default)).ReturnsAsync(new List<Genre>());
        mockMovieRepo.Setup(r => r.GetActorsAsync(default)).ReturnsAsync(new List<Actor>());
        mockMovieRepo.Setup(r => r.GetDirectorsAsync(default)).ReturnsAsync(new List<Director>());

        var service = new SlotMachineService(mockStateRepo.Object, mockMovieRepo.Object, new Mock<IEventRepository>().Object, new Mock<IUserMovieDiscountRepository>().Object, new Mock<INotificationRepository>().Object);
    ...
    public async Task GetAvailableSpinsAsync_ResetsSpins_WhenDateIsInPast()
    {
        ...
        var service = new SlotMachineService(mockStateRepo.Object, new Mock<IMovieRepository>().Object, new Mock<IEventRepository>().Object, new Mock<IUserMovieDiscountRepository>().Object, new Mock<INotificationRepository>().Object);
    ...
    public async Task WrapperMethods_ReturnExpectedValues()
    {
        ...
        var service = new SlotMachineService(mockStateRepo.Object, mockMovieRepo.Object, mockEventRepo.Object, new Mock<IUserMovieDiscountRepository>().Object, new Mock<INotificationRepository>().Object);
